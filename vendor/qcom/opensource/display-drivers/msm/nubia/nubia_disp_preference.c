...
static bool has_init_hbm_feedback_wq = false;
wait_queue_head_t hbm_feedback_event;
        ...
        struct kobj_attribute *attr, const char *buf, size_t size)
{
	...
	SDE_ATRACE_BEGIN("hbm_state_store");
	if(nubia_display == NULL) {
		NUBIA_DISP_ERROR("no nubia_display node!\n");
		return size;
	}
	...
	NUBIA_DISP_INFO("HBM state value %d\n", val);

	nubia_disp_val.hbm_state = val;

	SDE_ATRACE_END("hbm_state_store");
	return size;
}

static ssize_t hbm_feedback_show(struct kobject *kobj,
        struct kobj_attribute *attr, char *buf)
{
	if(nubia_display == NULL) {
		NUBIA_DISP_ERROR("no nubia_display node!\n");
	...
	NUBIA_DISP_INFO("Waiting hbm state feedback \n");
	wait_event(hbm_feedback_event, nubia_display->panel->hbm_change_done);
	NUBIA_DISP_INFO("hbm change done!!!%d\n", nubia_disp_val.hbm_state);
	nubia_display->panel->hbm_change_done = false;

	return snprintf(buf, PAGE_SIZE, "%d\n", nubia_disp_val.hbm_state);
}

static ssize_t hbm_feedback_store(struct kobject *kobj,
        struct kobj_attribute *attr, const char *buf, size_t size)
{
	return size; /* TO DO */
}

static ssize_t dimming_mode_show(struct kobject *kobj,
        struct kobj_attribute *attr, char *buf)
{
	if(nubia_display == NULL) {
		NUBIA_DISP_ERROR("no nubia_display node!\n");
	...
	return snprintf(buf, PAGE_SIZE, "%d\n", nubia_disp_val.dimming_mode);
}

static ssize_t dimming_mode_store(struct kobject *kobj,
        struct kobj_attribute *attr, const char *buf, size_t size)
{
	uint32_t val = 0;
	int ret = 0;
	...
	sscanf(buf, "%d", &val);
	if ((val != 0) && (val != 1)) {
		NUBIA_DISP_ERROR("invalid dimming mode val = %d\n", val);
		return size;
	}

	nubia_disp_val.dimming_mode = val;

	ret = nubia_enable_dimming_feature(nubia_display->panel,val);
	if(ret == 0)
		NUBIA_DISP_INFO("success to set dimming_mode as = %d\n", val);
        ...
        struct kobj_attribute *attr, const char *buf, size_t size)
{
	...
	sscanf(buf, "%d", &val);
	if ((nubia_display->panel->ddic_vendor == NOVATEK) || (nubia_display->panel->ddic_vendor == RAYDIUM)){
		if(val < 336 || val > 4095) {
	...
	struct kobj_attribute *attr, const char *buf, size_t size)
{
	...
	if(!nubia_display->panel->panel_initialized) {
...
static struct kobj_attribute lcd_disp_attrs[] = {
	...
	__ATTR(hbm_feedback, 0664, hbm_feedback_show, hbm_feedback_store),
	__ATTR(dimming_mode, 0664, dimming_mode_show, dimming_mode_store),
};
...
int __init nubia_disp_preference_init(void)
{
	int retval = 0;
	int attr_count = 0;
	...
	if(!has_init_hbm_feedback_wq) {
		init_waitqueue_head(&hbm_feedback_event);
	}
	has_init_hbm_feedback_wq = true;
...
void __exit nubia_disp_preference_exit(void)
{
	int attr_count = 0;
