								         ...
								         int value)
{
	...
	if (display->panel->ddic_vendor == VIEWTRIX) {
		if (value >= 336 && value <= 4080)
			...
			bl_lvl = value;
	} else if (display->panel->ddic_vendor == RAYDIUM) {
		if (value >= 336 && value <= 4080)
...
static int sde_backlight_device_update_status(struct backlight_device *bd)
{
	...
	if ((display->panel->hbm_mode) || (display->panel->aod_mode)) {
...
static int _sde_connector_update_bl_scale(struct sde_connector *c_conn)
{
	...
	SDE_DEBUG("bl_scale = %u, bl_scale_sv = %u, bl_level = %u\n",
		...
		bl_config->bl_level);
#ifdef CONFIG_NUBIA_DISP_PREFERENCE
	if ((bl_config->bl_level < 115) || (dsi_display->panel->hbm_mode) || (dsi_display->panel->aod_mode)) {
		c_conn->unset_bl_level = 0;
		ltm_status = 0;
		return 0;
	} else {
		...
		uint32_t len, uint32_t val)
{
	...
	SDE_INFO("connector:%d hw recovery event(%d) value (%d) notified\n",
