...
static void complete_commit(struct msm_commit *c)
{
	...
	if (hbm_mode_state != hbm_state_tmp) {
		if (hbm_state_tmp != 4095) {
			snprintf(hbm_name, sizeof(hbm_name), "hbm_disable_commit:%d", hbm_state_tmp);
			SDE_ATRACE_BEGIN(hbm_name);
			nubia_dsi_panel_hbm(nubia_display->panel, hbm_state_tmp);
	...
	if (hbm_mode_state != hbm_state_tmp) {
		if (hbm_state_tmp == 4095) {
			snprintf(hbm_name, sizeof(hbm_name), "hbm_enable_commit:%d", hbm_state_tmp);
			SDE_ATRACE_BEGIN(hbm_name);
			nubia_dsi_panel_hbm(nubia_display->panel, hbm_state_tmp);
			hbm_mode_state = hbm_state_tmp;
			SDE_ATRACE_END(hbm_name);
