...
char monitor_white_list[][20]={
	...
	{"LS27R75"},
	{"C35SQPLUS"},
	{"Mi 245HF2"}, // Mi monitor
	{"Mi TVPQ"}, // Mi TV
	{"M27N2S"}, // RedMagic monitor GM003
	{"27A6V"}, // RedMagic monitor GM001
};
char productvdo_blacklist[][20]={
	{"0x20030001"}, // Stiger hdmi cable
	{"0x00000115"}, // Philips hdmi cable
	{"0x01030001"}, // Pisen or UNI hdmi cable
};
...
void abandon_space_tab_str(char *str)
{
	...
	SDE_INFO("[msm-dp]%s: count = %d, buff_name = %s, monitor_name = %s\n",
			__func__, count, buff, str);
}

static inline void sde_edid_store_best_format(int vrefresh, int hdispay,
					      int vdispay, int ratio)
{
	...
	SDE_DEBUG("[msm-dp]%s: fps = %d, h = %d, v = %d, ratio = %d\n",
			__func__, vrefresh, hdispay, vdispay, ratio);
}

static void sde_edid_get_best_format(struct drm_display_mode *mode,
				     struct drm_connector *connector, int count)
{
	int h = 0, v = 0, v0 = 0;
	int fps = 0;
	int ratio = 0;
	...
	char *dp_productvdo = dp_get_productvdo();
	...
	list_for_each_entry (mode, &connector->modes, head) {
		...
		*/
		...
		//nubia filter for Mi
		if (!strncmp(edid_device_name, monitor_white_list[5],
				strlen(monitor_white_list[5]))) {
			if (h == 1920 && v == 1440 && vrefresh == FPS_120) {
				continue;
			}
			if (h == 1920 && v == 1080 && vrefresh == FPS_165) {
				continue;
			}
		}
		// nubia filter stiger hdmi cable and Mi monitor
		if (!strncmp(edid_device_name, monitor_white_list[6],
				strlen(monitor_white_list[6])) &&
				!strncmp(dp_productvdo, productvdo_blacklist[0],
				strlen(productvdo_blacklist[0]))) {
			if (h == 4096 && v == 2160 && vrefresh == FPS_60) {
				continue;
			}
			if (h == 3840 && v == 2160 && vrefresh == FPS_60) {
				continue;
			}
		}
		//nubia filter for Redmagic monitor GM003 no voice
		if (!strncmp(edid_device_name, monitor_white_list[7],
				strlen(monitor_white_list[7]))) {
			if (h == 2560 && v == 1440 && vrefresh == FPS_144)
				continue;
		}
		//nubia filter for Redmagic monitor GM001 no voice
		if (!strncmp(edid_device_name, monitor_white_list[8],
				strlen(monitor_white_list[8]))) {
			if ((h == 2560 && v == 1440 && vrefresh == FPS_144) ||
				(h == 2560 && v == 1440 && vrefresh == FPS_60) ||
				(h == 1920 && v == 1080 && vrefresh == FPS_120) ||
				(h == 1920 && v == 1080 && vrefresh == FPS_60))
				continue;
		}
		//nubia filter for Philips hdmi cable and Redmagic monitor
		if (!strncmp(edid_device_name, monitor_white_list[7],
				strlen(monitor_white_list[7])) &&
				!strncmp(dp_productvdo, productvdo_blacklist[1],
				strlen(productvdo_blacklist[1]))) {
			if (h == 1920 && v == 1080 && vrefresh == FPS_144)
				continue;
		}
		// nubia filter Pisen hdmi cable and Mi monitor
		if (!strncmp(edid_device_name, monitor_white_list[5],
				strlen(monitor_white_list[5])) &&
				!strncmp(dp_productvdo, productvdo_blacklist[2],
				strlen(productvdo_blacklist[2]))) {
			if (h == 1920 && v == 1080 && vrefresh == FPS_240)
				continue;
		}
		/*
		...
		*/
		if (!v0) {
			...
			SDE_INFO("[msm-dp]%s: use frist edid info, fps = %d, "
					"h = %d, v = %d, ratio = %d\n", __func__,
					vrefresh, h, v, mode->picture_aspect_ratio);
			sde_edid_store_best_format(vrefresh, h, v,
						   mode->picture_aspect_ratio);
		}
		/*select 16:9 && fps > 60 && v > 1080 && h > 1920*/
		ratio = h * 9 / 16;
		if (vrefresh >= FPS_60 && (ratio == v) &&
		    (v >= V_MIN && h >= H_MIN)) {
			if (vrefresh > fps) { //select max fps
				fps = vrefresh;
				SDE_INFO("[msm-dp]%s: vrefresh > fps, fps = %d, "
					"h = %d, v = %d, ratio = %d\n", __func__,
					vrefresh, h, v, mode->picture_aspect_ratio);
				sde_edid_store_best_format(
					vrefresh, h, v,
					mode->picture_aspect_ratio);
			} else if (vrefresh ==
				   fps) { //if fps is equal, select max resulation
				if (v > select_sde_edid_info.v &&
				    h > select_sde_edid_info.h) {
					fps = vrefresh;
					SDE_INFO("[msm-dp]%s: vrefresh == fps, fps= %d, "
						"h = %d, v = %d, ratio = %d\n", __func__,
						vrefresh, h, v, mode->picture_aspect_ratio);
			...
			if (!strncmp(&eld[DRM_ELD_MONITOR_NAME_STRING],
				     monitor_white_list[1], len1)) {
				SDE_INFO("[msm-dp]%s: monitor white list[1] = %s\n",
						__func__, monitor_white_list[1]);
				if (vrefresh == FPS_165 && v == V_MIN &&
				    h == H_MIN) {
					SDE_INFO("[msm-dp]%s: vrefresh == FPS_165 "
						"&& v == V_MIN && h == H_MIN, fps = %d, "
						"h = %d, v = %d, ratio = %d\n", __func__,
						vrefresh, h, v, mode->picture_aspect_ratio);
					sde_edid_store_best_format(
						vrefresh, h, v,
						mode->picture_aspect_ratio);
					break;
				} else if (vrefresh == FPS_120 && v == V_MIN &&
					   h == H_MIN) {
					SDE_INFO("[msm-dp]%s: vrefresh == FPS_120 "
						"&& v == V_MIN && h == H_MIN, fps = %d, "
						"h = %d, v = %d, ratio = %d\n", __func__,
						vrefresh, h, v, mode->picture_aspect_ratio);
					sde_edid_store_best_format(
						...
						mode->picture_aspect_ratio);
					break;
				} else if (vrefresh == FPS_240 && v == V_MIN &&
					   h == H_MIN) {
					SDE_INFO("[msm-dp]%s: vrefresh == FPS_240 "
						"&& v == V_MIN && h == H_MIN, fps = %d, "
						"h = %d, v = %d, ratio = %d\n", __func__,
						vrefresh, h, v, mode->picture_aspect_ratio);
					sde_edid_store_best_format(
			...
			} else if (!strncmp(&eld[DRM_ELD_MONITOR_NAME_STRING],
					    monitor_white_list[0], len2)) {
					SDE_INFO("[msm-dp]%s: monitor white list[0] = %s\n",
							__func__, monitor_white_list[0]);
				/**
			***	for data card GC553
			...
			***/
				...
				if (is_big_1080p ||
				    (nubia_global_cc_orientation & IVR_9)) {
					if (vrefresh == FPS_120 && v == V_MIN &&
					    h == H_MIN) {
						SDE_INFO("[msm-dp]%s: vrefresh == FPS_120 "
							"&& v == V_MIN && h == H_MIN, fps = %d, "
							"h = %d, v = %d, ratio = %d\n", __func__,
					...
					if (vrefresh == FPS_144) {
						SDE_INFO("[msm-dp]%s: vrefresh == FPS_144, fps = %d, "
							"h = %d, v = %d, ratio = %d\n", __func__,
							vrefresh, h, v, mode->picture_aspect_ratio);
						sde_edid_store_best_format(
							vrefresh, h, v,
					    ...
					    monitor_white_list[2], len3)) {
				SDE_INFO("[msm-dp]%s: monitor white list[2] = %s\n",
						__func__, monitor_white_list[2]);
				if (vrefresh == FPS_120 && v == V_MIN &&
				    h == H_MIN) {
					SDE_INFO("[msm-dp]%s: vrefresh == FPS_120 "
						"&& v == V_MIN && h == H_MIN, fps = %d, "
			...
			} else if (vrefresh == FPS_144 && v == V_MIN &&
				   h == H_MIN) {
				SDE_INFO("[msm-dp]%s: monitor white list[3] = %s\n",
						__func__, monitor_white_list[3]);
				fps = vrefresh;
				SDE_INFO("[msm-dp]%s: vrefresh == FPS_144 "
					"&& v == V_MIN && h == H_MIN, fps = %d, "
					"h = %d, v = %d, ratio = %d\n", __func__,
					vrefresh, h, v, mode->picture_aspect_ratio);
				sde_edid_store_best_format(
	...
	SDE_INFO("[msm-dp]%s: select_sde_edid_info: %d x %d %d %d\n",
			__func__, select_sde_edid_info.h,
			select_sde_edid_info.v, select_sde_edid_info.fps,
			select_sde_edid_info.ratio);
}

static void sde_edid_set_mode_format(struct drm_connector *connector,
				     struct sde_edid_ctrl *edid_ctrl)
{
	...
	**/
	...
	list_for_each_entry (mode, &connector->modes, head) {
		count++;
		vrefresh = drm_mode_vrefresh(mode);
		SDE_INFO("[msm-dp]hdmi_edid_info[%d]: %s %d %d %d %d %d 0x%x\n", count,
		       mode->name, vrefresh, mode->picture_aspect_ratio,
		       mode->htotal, mode->vtotal, mode->clock, mode->flags);
		sde_edid_get_v_h(mode->name, &h, &v);
		//nubia add modify for philips 200 cannot touping 144 in 573
		if (!strncmp(&connector->eld[DRM_ELD_MONITOR_NAME_STRING],
			     monitor_white_list[1],
			     strlen(monitor_white_list[1]))) {
			if (vrefresh == FPS_144 && h == H_MIN &&
			    v == V_MIN &&
		...
		if (!strncmp(&connector->eld[DRM_ELD_MONITOR_NAME_STRING],
			     monitor_white_list[0],
			     strlen(monitor_white_list[0]))) {
			if (vrefresh == FPS_144 && h == H_MIN &&
			    v == V_MIN &&
			    (nubia_global_cc_orientation & IVR_9)) {
				continue;
		...
		if (!strncmp(&connector->eld[DRM_ELD_MONITOR_NAME_STRING],
			     monitor_white_list[2],
			     strlen(monitor_white_list[2]))) {
			if (vrefresh == FPS_144) {
				continue;
			}
		}
		//nubia filter 3440X1440 100HZ for TITAN ARMY
		if (!strncmp(edid_device_name, monitor_white_list[4],
			     strlen(monitor_white_list[4]))) {
			if (h == 3440 && v == 1440 && vrefresh == FPS_100) {
				continue;
			}
		}
		//nubia filter for Mi
		...
		// nubia filter stiger dp hdmi and Mi monitor
		if (!strncmp(edid_device_name, monitor_white_list[6],
				strlen(monitor_white_list[6])) &&
				!strncmp(dp_productvdo, productvdo_blacklist[0],
				strlen(productvdo_blacklist[0]))) {
		...
		if (!select_sde_edid_info.edid_mode_store) {
			ret = snprintf(
				select_sde_edid_info.edid_mode_info + len,
				max_size, "%s %d %d\n", mode->name,
				vrefresh, mode->picture_aspect_ratio);
			if (dp_check_buffer_overflow(ret, &max_size, &len))
				SDE_ERROR("[msm-dp]%s: buffer overflow, %d\n",
					       __func__, __LINE__);
			SDE_DEBUG("[msm-dp]%s: select_sde_edid_info.edid_mode_info = %s\n",
					__func__, select_sde_edid_info.edid_mode_info);
		}

		if (info_temp) {
			if ((info_temp->fps == vrefresh) &&
			    (info_temp->h == h) && (info_temp->v == v)) {
				SDE_INFO("[msm-dp]%s: alreay selected last time\n",
				       __func__);
				sde_edid_store_best_format(
					vrefresh, h, v,
					mode->picture_aspect_ratio);
	...
	if (!select_sde_edid_info.edid_mode_store && !has_found) {
		if (count > 0) {
			SDE_INFO("[msm-dp]%s: edid_mode_store = %d, has_found = %d\n",
					__func__, select_sde_edid_info.edid_mode_store, has_found);
...
bool sde_detect_hdmi_monitor(void *input)
{
	...
	select_sde_edid_info.hdmi_connected = true;
	SDE_EDID_DEBUG("%s hdmi connected\n", __func__);
	return ret;
#else
