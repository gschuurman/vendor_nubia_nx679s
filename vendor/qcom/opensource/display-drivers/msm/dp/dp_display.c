...
#ifdef CONFIG_NUBIA_HDMI_FEATURE
	t = wait_for_completion_timeout(&dp->notification_comp, HZ * 10);
...
static int dp_display_usbpd_disconnect_cb(struct device *dev)
{
	...
	DP_INFO("edid_hot_plug = %d, edid_mode_store = %d\n",
			select_sde_edid_info.edid_hot_plug,
			select_sde_edid_info.edid_mode_store);
#endif
...
#ifdef CONFIG_NUBIA_HDMI_FEATURE
		DP_INFO("dp audio attention work enter\n");
#endif
...
#ifdef CONFIG_NUBIA_HDMI_FEATURE
	dp_display_state_log("[hdcp aborted]");
#endif
	cancel_delayed_work_sync(&dp->hdcp_cb_work);
	if (dp_display_is_hdcp_enabled(dp) &&
			status->hdcp_state != HDCP_STATE_INACTIVE) {
...
#ifdef CONFIG_NUBIA_HDMI_FEATURE
	dp_display_state_log("[hdcp not aborted]");
#endif
	for (i = DP_STREAM_0; i < DP_STREAM_MAX; i++) {
		if (dp->active_panels[i]) {
			if (status->hdcp_state != HDCP_STATE_AUTHENTICATED)
		...
		const struct msm_resource_caps_info *avail_res)
{
	...
	if (!select_sde_edid_info.node_control) {
		...
		if (flag) {
			debug_en = dp_panel->mode_override;
			vrefresh = dp_panel->vrefresh;
			hdisplay = dp_panel->hdisplay;
			vdisplay = dp_panel->vdisplay;
			flag = 0;
		}
		/*second: if hdmi disconnect, restore the default config*/
		if (!select_sde_edid_info.hdmi_connected) {
			dp_panel->mode_override = debug_en;
			dp_panel->vrefresh = vrefresh;
			dp_panel->hdisplay = hdisplay;
			dp_panel->vdisplay = vdisplay;
			dp_panel->aspect_ratio = aspect_ratio;
		}
		/*if hdmi connected, we chose best resultion and fps*/
		if (select_sde_edid_info.hdmi_connected) {
			dp_panel->mode_override = true;
			dp_panel->vrefresh = select_sde_edid_info.fps;
			dp_panel->hdisplay = select_sde_edid_info.h;
			dp_panel->vdisplay = select_sde_edid_info.v;
			dp_panel->aspect_ratio =select_sde_edid_info.ratio;
		}
	}
#endif
