				...
				struct kobj_attribute *attr, char *buf)
{
	...
	if ((global_nubia_dp_aux->state & DP_STATE_TRAIN_1_SUCCEEDED) &&
	    (global_nubia_dp_aux->state & DP_STATE_TRAIN_2_SUCCEEDED))
		return snprintf(buf, PAGE_SIZE, "%d\n", 1);
	else
		return snprintf(buf, PAGE_SIZE, "%d\n", -1);
}
				  ...
				  size_t size)
{
	...
	DP_INFO(": %s\n", debug_node->hotplug ? "debug hpd connect" : "debug hpd disconnect");
	if (hpd == 0)
			       ...
			       struct kobj_attribute *attr, char *buf)
{
	...
	len = snprintf(buf_edid + ret, max_size, "%s",
		       select_sde_edid_info.edid_mode_info);
	DP_INFO("edid_mode_best_info = %s\n", edid_mode_best_info);
...
{
	DP_INFO("h = %d, v = %d, fps = %d, name=%s\n", info_temp->h,
	       info_temp->v, info_temp->fps, info_temp->device_name);
}

void sys_show_edid_info(void)
{
	struct _user_select_sde_edid_info *info_temp = edid_info;
	DP_INFO("to show edid info\n");
	for (; info_temp;) {
...
{
	struct _user_select_sde_edid_info *info_temp = edid_info;
	DP_INFO("sys_store_edid_info: h = %d, v = %d, fps = %d, ratio = %d\n",
			h, v, fps, rto);
	if (!edid_info) {
				...
				size_t size)
{
	...
	debug_node->panel->mode_override = true;
	debug_node->panel->hdisplay = hdisplay;
	debug_node->panel->vdisplay = vdisplay;
	debug_node->panel->vrefresh = vrefresh;
	debug_node->panel->aspect_ratio = aspect_ratio;
	...
	snprintf(edid_mode_best_info, 32, "%dx%d %d %d\n", hdisplay, vdisplay,
		 vrefresh, aspect_ratio);

	strcpy(device_name, edid_device_name);
	sys_store_edid_info(hdisplay, vdisplay, vrefresh, aspect_ratio,
			    device_name);
...
clear:
	...
	debug_node->panel->mode_override = false;
end:
	return size;
}
					    ...
					    const char *buf, size_t size)
{
	...
	if (sscanf(buf, "%d %d %d %d %s %s", &hdisplay, &vdisplay, &vrefresh,
		   &aspect_ratio, device_name, sub_name) <= 0)
		goto end;
					   ...
					   char *buf)
{
	...
	for (; info_temp;) {
		snprintf(edid_modes_list, 32, "%dx%d %d %d %s\n", info_temp->h,
			 info_temp->v, info_temp->fps, info_temp->ratio,
			 info_temp->device_name);
		ret += snprintf(buf_edid + ret, max_size, "%s",
				edid_modes_list);
		info_temp = info_temp->next;
	}
	DP_INFO("edid_mode_best_info = %s\n", buf_edid);
...
static struct kobj_attribute disp_attrs[] = {
	...
	__ATTR(selected_edid, 0664, dp_debug_selected_edid_show,
	       dp_debug_selected_edid_store),
};
...
int __init nubia_dp_preference_init(void)
{
	...
	for (attr_count1 = 0; attr_count1 < ARRAY_SIZE(usb_disp_attrs);
	     attr_count1++) {
		retval1 = sysfs_create_file(enhance_kobj,
					    &usb_disp_attrs[attr_count1].attr);
		if (retval1 < 0) {
			goto err_sys_creat1;
		}
	}
	...
	for (attr_count2 = 0; attr_count2 < ARRAY_SIZE(disp_attrs);
	     attr_count2++) {
		retval2 = sysfs_create_file(nubia_global_enhace_kobj,
					    &disp_attrs[attr_count2].attr);
		if (retval2 < 0) {
			goto err_sys_creat2;
		}
	}
...
err_sys_creat1:
	for (--attr_count1; attr_count1 >= 0; attr_count1--)
		sysfs_remove_file(enhance_kobj,
				  &usb_disp_attrs[attr_count1].attr);
...
void __exit nubia_dp_preference_exit(void)
{
	...
	kobject_put(enhance_kobj);
	for (attr_count2 = 0; attr_count2 < ARRAY_SIZE(usb_disp_attrs);
	     attr_count2++)
