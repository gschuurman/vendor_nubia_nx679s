...
static int nubia_parse_switcher_dt(struct device_node *node)
{
	...
	pswitcher->switch_en =
		of_get_named_gpio_flags(node, "qcom,switch-en-gpio", 0, NULL);
	...
	rc = gpio_direction_output(pswitcher->switch_en,
				   pswitcher->switch_en_flag);
	if (rc < 0) {
		dev_err(pswitcher->dev,
			"parse switch_en_direction_output error\n");
		return rc;
	}

	pswitcher->switch_mode =
	...
	rc = gpio_direction_output(pswitcher->switch_mode,
				   pswitcher->switch_mode_flag);
	if (rc) {
	...
	pr_info("[msm-dp-info]%s:pswitcher->switch_en = %d, pswitcher->switch_mode = %d\n",
...
static ssize_t dp_cc_statue_store(struct device *dev,
				  ...
				  const char *buf, size_t count)
{
	...
	pr_info("[msm-dp-info]%s please not set cc_statue!\n", __func__);
	return count;
}
...
static DEVICE_ATTR(cc_statue, S_IWUSR | S_IRUGO, dp_cc_statue_show,
		   dp_cc_statue_store);
...
static int nubia_usb_switch_dp_probe(struct platform_device *pdev)
{
	...
	if (rc < 0)
		return rc;
	ret = sysfs_create_group(&pswitcher->dev->kobj,
...
static int nubia_usb_switch_dp_remove(struct platform_device *pdev)
{
	if (!pswitcher)
		return -EINVAL;
	sysfs_remove_group(&pswitcher->dev->kobj,
			   &usb_switch_dp_attribute_group);
	return 0;
}
