		...
		void *display, u32 bl_lvl)
{
	...
	if ((bl_temp > panel->bl_config.bl_max_level) && (bl_temp != 4095))
		...
		for (i = 0; i < len; ++i) {
			if (config->return_buf[i] !=
				...
				if (panel->ddic_vendor == RAYDIUM) {
					if (i == 0) {
						DRM_ERROR("mismatch[C5]: 0x%x, mismatch[0A]: 0x%x,status_value: 0x%x, i:%d \n",
							config->return_buf[i],config->return_buf[i+1] ,config->status_value[group + i], i);
					} else if (i == 1) {
						DRM_ERROR("mismatch[C5]: 0x%x, mismatch[0A]: 0x%x,status_value: 0x%x, i:%d \n",
							config->return_buf[i-1],config->return_buf[i] ,config->status_value[group + i], i);
						break;
					} else {
...
#ifdef CONFIG_NUBIA_DISP_PREFERENCE
int nubia_check_demura_status(struct dsi_display_ctrl *ctrl, struct dsi_panel *panel)
{
	int rc = 0;
	 ...
	 */
	if (!dsi_ctrl_validate_host_state(ctrl->ctrl)) {
		DSI_ERR("DSI controller is not in initialized state.\n");
		return 1;
	}
		...
		struct dsi_display *display)
{
	...
	if (display->panel->ddic_vendor == VIEWTRIX) {
		...
		rc = nubia_check_demura_status(ctrl, display->panel);
	}
#endif
...
static int dsi_display_debugfs_init(struct dsi_display *display)
{
	...
	if (!debugfs_create_bool("mrc_bypass", 0600, dir,
			&display->panel->mrc_bypass)) {
		DSI_ERR("[%s] debugfs create mrc_bypass feature file failed\n",
			   display->name);
		goto error_remove_dir;
	}
#endif
					...
					unsigned int display_type)
{
	...
	NUBIA_DEBUG("successfully parsed command line topology and timing\n");
end:
...
static int dsi_display_parse_boot_display_selection(void)
{
	...
	for (i = 0; i < MAX_DSI_ACTIVE_DISPLAY; i++) {
       ...
       // if (i == 0)
             //strcpy(boot_displays[i].boot_param, "qcom,mdss_dsi_visionox_r66451_fhd_plus_cmd:");// "qcom,mdss_dsi_sim_video");// // "qcom,qcom,dsi_r66451_amoled_120hz_cmd_cphy:");//mdss_dsi_r6130_1080_2400_amoled_cmd
				...
				const struct mipi_dsi_msg *msg)
{
	...
	if (!host || !msg) {
		DSI_ERR("[%s] dsi_host_transfer Invalid params\n", __FUNCTION__);
		return 0;
	}
	...
	cmd.ctrl = 0;
	cmd.post_wait_ms = 0;
	cmd.ctrl_flags = cmd_flags;
	ctr = &display->ctrl[ctrl_idx];

	dsi_display_set_cmd_tx_ctrl_flags(display,&cmd);
	rc = dsi_ctrl_transfer_prepare(ctr->ctrl, cmd.ctrl_flags);
	if (rc) {
		DSI_ERR("prepare for rx cmd transfer failed rc=%d\n", rc);
		return rc;
	}

	rc = dsi_ctrl_cmd_transfer(ctr->ctrl, &cmd);
	if (rc <= 0) {
		DSI_ERR("[%s] cmd transfer failed, rc=%d\n",
			display->name, rc);
		goto error_disable_cmd_engine;
	...
	dsi_ctrl_transfer_unprepare(ctr->ctrl, cmd.ctrl_flags);
...
static int _dsi_display_dev_init(struct dsi_display *display)
{
	...
	NUBIA_DEBUG("_dsi_display_dev_init ++: %s\n", display->name);
...
int dsi_display_dev_probe(struct platform_device *pdev)
{
	...
	pr_err("dsi_display_dev_probe ++\n");

    NUBIA_DEBUG("8450 dsi_display_dev_probe [%s] , pdev->dev.of_node->name is %s, pdev->dev.of_node->full_name is %s\n",pdev->name,pdev->dev.of_node->name,pdev->dev.of_node->full_name);
