...
#ifdef CONFIG_NUBIA_DISP_PREFERENCE
/*Raydium DDIC power on*/
static int rdm_panel_power_on(struct dsi_panel *panel)
{
	int rc = 0;

	NUBIA_DEBUG("%s rdm_panel_power_on %s ++ ", panel->name, __func__);
	...
	rc = nubia_dsi_pwr_enable_regulator(&panel->power_info, "vci", true);
	if (rc) {
		DSI_ERR("[%s] failed to enable vregs, rc=%d\n",
				panel->name, rc);
		goto exit;
...
static int vtdr_panel_power_on(struct dsi_panel *panel)
{
	...
	rc = nubia_dsi_pwr_enable_regulator(&panel->power_info, "vci", true); //modify vdd to vci
	if (rc) {
		DSI_ERR("[%s] failed to enable vregs, rc=%d\n",
				panel->name, rc);
		goto exit;
...
/*Raydium DDIC power off*/
static int rdm_panel_power_off(struct dsi_panel *panel)
{
	int rc = 0;

	NUBIA_DEBUG("%s rdm_panel_power_on %s ++ ", panel->name, __func__);
	rc = nubia_dsi_pwr_enable_regulator(&panel->power_info, "vci", false);
	if (rc) {
		DSI_ERR("[%s] failed to enable vregs, rc=%d\n",
				panel->name, rc);
		goto exit;
...
static int vtdr_panel_power_off(struct dsi_panel *panel)
{
	...
	rc = nubia_dsi_pwr_enable_regulator(&panel->power_info, "vci", false); //modify vdd to vci
	if (rc) {
		DSI_ERR("[%s] failed to enable vregs, rc=%d\n",
				panel->name, rc);
		goto exit;
...
static int dsi_panel_power_off(struct dsi_panel *panel)
{
	...
	nubia_disp_val.dimming_mode = 0;
	panel->hbm_mode = false;
	panel->aod_mode = false;
#else
	...
	u32 bl_lvl)
{
	...
	if (panel->ddic_vendor == RAYDIUM) {
		if ((nubia_disp_val.panel_firmware_id != 0xa1) && (nubia_disp_val.panel_firmware_id != 0xa2))
			nubia_set_brightness_mrc(panel, bl_lvl);
	}

	SDE_ATRACE_INT("set_brightness", bl_lvl);
#endif
...
#ifdef CONFIG_NUBIA_DISP_PREFERENCE
	if (nubia_disp_val.dimming_mode == 0)
...
const char *cmd_set_prop_map[DSI_CMD_SET_MAX] = {
	...
	"nubia,mdss-dsi-demura-command",
	"nubia,mdss-dsi-change-page-command",
	"nubia,mdss-dsi-enable-hs-mode-switch-on-command",
	"nubia,mdss-dsi-disable-hs-mode-switch-on-command",
	"nubia,mdss-dsi-mrc1-command",
	"nubia,mdss-dsi-mrc2-command",
	"nubia,mdss-dsi-mrc3-command",
#endif
};
...
const char *cmd_set_state_map[DSI_CMD_SET_MAX] = {
	...
	"nubia,mdss-dsi-demura-command-state",
	"nubia,mdss-dsi-change-page-command-state",
	"nubia,mdss-dsi-enable-hs-mode-switch-on-command-state",
	"nubia,mdss-dsi-disable-hs-mode-switch-on-command-state",
	"nubia,mdss-dsi-mrc1-command-state",
	"nubia,mdss-dsi-mrc2-command-state",
	"nubia,mdss-dsi-mrc3-command-state",
#endif
};
...
static int dsi_panel_parse_gpios(struct dsi_panel *panel)
{
	...
	panel->reset_config.panel_dvdd_1p2_goio = utils->get_named_gpio(utils->data,
						...
						0);
	if (!gpio_is_valid(panel->reset_config.panel_dvdd_1p2_goio)) {
		DSI_ERR("gpio [%s] panel_dvdd_1p2_goio is not set, rc=%d\n",
			 panel->name, rc);
	}
#endif
...
int nubia_backlight_covert2(struct dsi_panel *panel, int value)
{
	...
	if (panel->ddic_vendor == VIEWTRIX) {
		if (value >= 336 && value <= 4080)
			...
			bl_lvl = value;
	} else if (panel->ddic_vendor == RAYDIUM) {
		if (value >= 336 && value <= 4080)
			bl_lvl = ((value * 516) / 1000) - 58;
		else
			bl_lvl = value;
	} else {
		value = value / 2;
...
static int nubia_set_aod_brightness_local(struct dsi_panel *panel, uint32_t value)
{
	...
	if ((panel->ddic_vendor == NOVATEK) || (panel->ddic_vendor == RAYDIUM)) {
...
int nubia_dsi_panel_hbm(struct dsi_panel *panel, uint32_t value)
{
	...
	if (value == 4095) {
		...
		panel->bl_config.bl_level = value;
		rc = dsi_panel_update_backlight(panel, value);
...
int nubia_set_brightness_mrc(struct dsi_panel *panel, u32 bl_lvl)
{
	int rc = 0;
	int mrc_type = 0;

	if (panel->mrc_bypass) {
		NUBIA_DEBUG("nubia_set_brightness_mrc bypass !\n");
		return rc;
	}

	if ((bl_lvl >= 0x73) && (bl_lvl <= 0xAC))
		mrc_type = MRC1;
	else if ((bl_lvl >= 0xAD) && (bl_lvl <= 0x110))
		mrc_type = MRC2;
	else if ((bl_lvl >= 0x111) && (bl_lvl <= 0xFFF))
		mrc_type = MRC3;
	else
		mrc_type = MRC_UNKNOWN;

	if (mrc_type == mrc_type_tmp)
		return rc;

	switch (mrc_type) {
		case MRC1:
			rc = dsi_panel_tx_cmd_set(panel, DSI_CMD_SET_MRC1);;
			if (rc) {
				DSI_ERR("[%s] failed to send DSI_CMD_SET_MRC1 cmds, rc=%d\n",
					panel->name, rc);
			}
			break;
		case MRC2:
			rc = dsi_panel_tx_cmd_set(panel, DSI_CMD_SET_MRC2);
			if (rc) {
				DSI_ERR("[%s] failed to send DSI_CMD_SET_MRC2 cmds, rc=%d\n",
					panel->name, rc);
			}
			break;
		case MRC3:
			rc = dsi_panel_tx_cmd_set(panel, DSI_CMD_SET_MRC3);
			if (rc) {
				DSI_ERR("[%s] failed to send DSI_CMD_SET_MRC3 cmds, rc=%d\n",
					panel->name, rc);
			}
			break;
		default:
...
void panel_aod_send_uevent(struct dsi_panel *panel, int mode, int ret)
{
	char *envp[3];

	if (mode == DSI_CMD_SET_NOLP)
		envp[0] = "AOD_STATUS=OFF";
	else
		envp[0] = "AOD_STATUS=ON";

	if (ret == 0)
		envp[1] = "AOD_SET_RESULT=AODENTER";
	else
		envp[1] = "AOD_SET_RESULT=AODEXIT";

	envp[2] = NULL;

	kobject_uevent_env(&panel->parent->kobj, KOBJ_CHANGE, envp);
	NUBIA_DEBUG("aod send uevent:%s ok ", envp[0]);
...
int dsi_panel_set_nolp(struct dsi_panel *panel)
{
	...
	SDE_ATRACE_END("exit aod");
	panel->aod_mode = false;
	NUBIA_DEBUG("exit aod mode, fps value = %d", cur_fps);
	panel_aod_send_uevent(panel, DSI_CMD_SET_NOLP, DSI_CMD_SET_ON);
#else
	rc = dsi_panel_tx_cmd_set(panel, DSI_CMD_SET_NOLP);
	if (rc)
		DSI_ERR("[%s] failed to send DSI_CMD_SET_NOLP cmd, rc=%d\n",
...
int dsi_panel_switch(struct dsi_panel *panel)
{
	...
	rc = dsi_panel_tx_cmd_set(panel, DSI_CMD_SET_ENABLE_HS_TIMING_SWITCH);
        if (rc)
                DSI_ERR("[%s] failed to send DSI_CMD_SET_ENABLE_HS_SWITCH cmds, rc=%d\n",
                       panel->name, rc);
	...
	rc = dsi_panel_tx_cmd_set(panel, DSI_CMD_SET_DISABLE_HS_TIMING_SWITCH);
	if (rc)
		DSI_ERR("[%s] failed to send DSI_CMD_SET_DSIABLE_HS_SWITCH cmds, rc=%d\n",
		       panel->name, rc);
...
int nubia_check_demura_flag(struct dsi_panel *panel)
{
	...
	rc = dsi_panel_tx_cmd_set(panel, DSI_CMD_SET_DEMURA);
	if (rc) {
		DSI_ERR("[%s] failed to send DSI_CMD_SET_DEMURA cmd, rc=%d\n",
		       panel->name, rc);
		return rc;
	}
	...
	rc = dsi_panel_tx_cmd_set(panel, DSI_CMD_SET_PAGE);
	if (rc) {
		DSI_ERR("[%s] failed to send DSI_CMD_SET_PAGE cmd, rc=%d\n",
		       panel->name, rc);
		goto error;
	}
...
void nubia_check_flash_demura(struct dsi_panel *panel)
{
	...
	if (rc < 0) {
		pr_err("%s: read flash data error \n", __func__);
		goto error;
	}
...
int dsi_panel_enable(struct dsi_panel *panel)
{
	...
	NUBIA_DEBUG("%s:send command start ++++\n", __func__);

	mutex_lock(&panel->panel_lock);

	NUBIA_DEBUG("send command start ++++\n");
